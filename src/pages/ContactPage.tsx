...
const ContactPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("inquiries").insert({
      ...
      type: typeMap[formData.subject] || "general",
    } as any);
    setSubmitting(false);
    if (error) {
        ...
        <div className="max-container max-w-2xl">
          <FadeIn>
            ...
            <div className="flex gap-6 text-sm text-muted-foreground mb-16">
              <a href="https://www.airbnb.com/h/gudaurichalet" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">Georgia on Airbnb ↗</a>
              <a href="https://www.airbnb.com/l/LEHC2J81" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">Atlantique on Airbnb ↗</a>
            </div>
          </FadeIn>
          ...
          <FadeIn delay={0.15}>
            {submitted ? (
              <div className="text-center py-16">
                <p className="font-display text-lg italic text-primary mb-2">Thank you for reaching out.</p>
                <p className="text-sm text-muted-foreground">Managed by Darya — Response within 24 hours.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <select value={formData.subject} onChange={(e) => setFormData({ ...formData, subject: e.target.value })} className="w-full px-4 py-3 border border-border bg-background text-foreground text-sm focus:outline-none focus:border-primary transition-colors">
